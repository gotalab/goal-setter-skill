...
Subagents are the parallel worker for separable investigation, source-backed research, review, and validation — and for write units whose files are cleanly partitioned. Read-only is the main use, not the only one (see Parallel Fan-out).

When subagents are intended, encode that in the Goal text as an explicit, imperative instruction to use them — never leave delegation implicit. On Codex a goal that is silent on subagents, or that only grants abstractly ("use subagents"), runs everything in-context; you must name the tool and the action ("spawn a read-only subagent to <X>"). Runtime policy may still restrict actual spawning, but the instruction must be present. This clause should survive Goal compression; make it short, not absent.
...
- **Codex**: both `spawn_agent` and `create_thread` fire only from a `/goal` line the *human sends*, not from a goal the skill auto-sets via `create_goal` (verified: the directive fired when the human typed `/goal`, was declined when tool-set). So for fan-out work, **do not auto-set — emit the `/goal …` line for the user to send**; that one send authorizes the cascade. Name the tools concretely and imperatively — abstract phrasing ("fresh-context verification", "use subagents") and hedges ("when useful", "only when separable") launch nothing (verified). Two rules keep the run from silently falling back to serial:
  ...
  - **Name the tool, not its arguments; don't over-promise `create_thread`.** Write `spawn_agent`/`create_thread` and "own worktree", but never `projectId`/`target.type`/schema fields — tool-call-shaped text overfits the schema and stalls the executor. Because `create_thread` needs a resolvable `projectId`, it is unavailable on a fresh workspace, so default write fan-out to `spawn_agent` over strictly partitioned files, and use `create_thread` only when an established project exists and units must share files. Each `create_thread` carries its own worktree **and a goal scoped to that unit** — the orchestrator (main thread) sets those per-thread goals and integrates the results; the human sends only the top-level `/goal`. Tell Codex to work in the current project/workspace and let the runtime resolve the location. Write the directive as an explicit imperative — "for each write unit, **open a separate thread with `create_thread`** (own worktree, a goal scoped to that unit) and run them in parallel" — and pair it with the anti-fallback clause that mirrors read-only's "do not self-review": **"do not implement the units one-by-one in the main thread."** Without that, Codex quietly builds them serially.
...
Before claiming Done on high-confidence completion, verify the evidence with an independent read-only subagent or equivalent — not self-review (on Codex, the imperative "spawn a read-only subagent to verify…", since the bare phrase "fresh-context check" launches nothing); add completeness criticism when coverage claims are broad.
