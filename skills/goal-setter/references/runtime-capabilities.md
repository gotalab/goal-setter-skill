...
In the Goal text, write the structure, not a spawn command. The structure works on both runtimes and even when the pieces are only discovered at runtime: a discovery rule for finding the independent units, an owned surface and its own Done evidence per unit, item-by-item progress, and a parent integration check over the merged result. State the rule rather than enumerating pieces you cannot yet name. A `/goal` does not spawn parallel runs on its own, and — verified — a spawn instruction placed in the Goal text does not trigger Codex to parallelize. So the Goal carries the contract, not the fan-out command; name owned surfaces, cap the parallel count and retries, and gate parent Done on every unit's evidence plus the integration check.

How the parallel execution is triggered, per runtime:
...
- **Codex**: the Goal text does **not** trigger parallelism — verified. Codex launches parallel work only on an explicit user request in the prompt, via two user-triggered tools: `spawn` subagents ("spawn one agent per module") — parallel workers the main agent orchestrates and whose results it collects, with no separate goals; or `create_thread` ("create a separate thread per module") — genuinely separate threads, optionally each in its own git worktree, each given an initial instruction that can set its own goal, again orchestrated by the main thread. Because neither fires from Goal text, goal-setter hands the user a short paste-line to send as their prompt (see Output Style in `SKILL.md`); the Goal itself stays a sequential contract that coexists with whichever mechanism the user triggers.
