...
This work splits into independent, separately verifiable sub-outcomes that share no state, so spawn one agent per piece — on Codex give each its own `create_goal` contract scoped to its files, on Claude Code fan out through a dynamic workflow — each owning only its surface, the parallel count and retries capped, and Done gated on every piece's evidence plus an integration check that confirms the merged result. (Drop this whole paragraph when the work is not decomposable — interlocking files, a single-cause investigation, or a serially tuned metric — and keep one write contract, using read-only subagents only for separable investigation.)
