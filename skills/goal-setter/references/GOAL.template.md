...
This work splits into independent, separately verifiable sub-outcomes that share no state. Treat each discovered unit as a separately verifiable piece that owns only its files, work unit by unit, and gate Done on every unit's evidence plus an integration check that confirms the merged result. Large work often stages as a phased pipeline: (0) the main thread bootstraps a shared baseline, (1) parallel read-only research to clarify scope, (2) parallel write implementation, (3) integrate, (4) parallel adversarial/final verification — use the phases that fit. On Claude Code the run may fan out through a dynamic workflow on its own judgment — subagents for read-only stages, worktree isolation for write stages. On Codex, orchestrate from this main thread, following three rules: **(1)** if this workspace is empty or not yet git-backed, first bootstrap it here — git init, build/test scaffold, committed cross-module interface contracts (phase 0) — before any write fan-out. **(2)** Spawn read-only subagents (`spawn_agent`) to research/review/verify (do not self-review), and fan out write units in parallel — `spawn_agent` over strictly partitioned files, or `create_thread` per unit when an established project exists (it needs a resolvable `projectId`, so it is unavailable on a fresh workspace). **(3)** Name the tool, not its arguments: do not specify `projectId`, `target.type`, or other tool-schema fields; work in the current project/workspace and let the runtime resolve the location. Integrate and validate here, autonomously without waiting for further user instruction. (Drop this whole paragraph when the work is not decomposable — interlocking files, a single-cause investigation, or a serially tuned metric — and keep one write contract, using read-only subagents only for separable investigation.)
