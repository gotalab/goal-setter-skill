...
This work splits into independent, separately verifiable sub-outcomes that share no state. Treat each discovered unit as a separately verifiable piece that owns only its files, work unit by unit, and gate Done on every unit's evidence plus an integration check that confirms the merged result. For parallel speed: on Claude Code the run may fan out through a dynamic workflow on its own judgment; on Codex, orchestrate from this main thread — for each unit use the `create_thread` tool to open a separate thread in its own worktree, set it a goal scoped to only that unit, run them in parallel, then integrate and validate in this main thread, autonomously without waiting for further user instruction. (Drop this whole paragraph when the work is not decomposable — interlocking files, a single-cause investigation, or a serially tuned metric — and keep one write contract, using read-only subagents only for separable investigation.)
