...
Every non-trivial Goal should explicitly and imperatively instruct use of available governed subagents for separable investigation, research, validation discovery, triage, strategy review, and final fresh-context review; read-only is the main use, but subagents also cover write units with cleanly partitioned files. Keep the instruction short but present, and never omit it: on Codex a goal that is silent — or that only grants abstractly ("use subagents") — runs everything in-context, so name the tool and action; Claude Code fans out on its own judgment regardless. For parallel fan-out (decomposition, multi-aspect review, multi-topic research), put the decomposition structure in the Goal (see §Inline Goal Condition) and size the launch directive to the runtime; the runtime mechanics — Claude Code's dynamic workflow, and Codex's user-sent `/goal` line with subagent/`create_thread` rules — live in `references/runtime-capabilities.md`.
