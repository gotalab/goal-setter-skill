...
Every non-trivial Goal should explicitly allow available governed subagents for separable investigation, source-backed research, validation discovery, test-failure triage, strategy review when progress stalls, and final fresh-context review. Prefer read-only subagents unless an external side effect is required, allowed by policy, and part of end-to-end completion. Keep the authorization short but present. It mainly shapes Claude Code, where the model fans out on its own judgment. On Codex both `spawn_agent` (subagents — read-only, and write units with partitioned files) and `create_thread` (own worktree, only when an established project exists) are gated to the user's own typed request: a goal the skill auto-sets via `create_goal` does not trigger them, but a `/goal …` line the user types does (verified). So on Codex, deliver decomposable goals as a `/goal …` line for the user to send — that one send authorizes the cascade, with the human's send as the trigger.

For parallel fan-out — implementation decomposition, multi-aspect review, or multi-topic research — put the structure in the Goal: a discovery rule for the independent units (state the rule, do not enumerate units unknown at draft time), an owned surface and its own Done evidence or deliverable per unit, item-by-item progress, and a parent integration/synthesis check. The structure works on both runtimes and even when the units are discovered at runtime. Large work often stages as a phased pipeline: (0) the main thread bootstraps a shared baseline, (1) parallel read-only research, (2) parallel write implementation, (3) integrate, (4) parallel adversarial/final verification — use the phases that fit. For parallel speed, add a launch directive sized to the runtime. On Claude Code the run fans out via a dynamic workflow on its own judgment (subagents for read-only stages, worktree isolation for write stages). On Codex, write flat-imperative directives, naming the tool concretely (abstract phrasing or hedges like "when useful" launch nothing — verified). Three Codex rules, all learned from real runs:

- **Subagents are the default.** `spawn_agent` is always available; use it for read-only work ("spawn read-only subagents to research/review/verify <X>; do not self-review") and for write units whose files are cleanly partitioned. `create_thread` (own worktree) is only available when an established project exists — it needs a resolvable `projectId` and cannot be cut on a fresh/empty workspace — so reserve it for write units that must share files, and default everything else to partitioned subagents.
- **Name the tool, not its arguments.** Never write `projectId`, `target.type`, or other tool-schema fields into the Goal — a `projectId` the executor cannot see is read as "cannot create a thread" and it falls back to serial (observed). Tell Codex to work in the current project/workspace and let the runtime resolve the location.
- **Bootstrap first (phase 0).** When the workspace may be empty or not yet git-backed, write the ordering: the main thread first bootstraps the repo (git init, build/test scaffold, committed cross-module **interface contracts**), then fans out write units against that fixed baseline. Do not fan out into bare ground.

Decomposability is *your* draft-time judgment: once you have judged the work decomposable, the Goal **commands** the fan-out — the "decompose only when…" conditions are your pre-draft checklist, not words to copy in. On Codex, deliver the goal as a `/goal …` line for the user to send rather than auto-setting it via `create_goal` (a tool-set goal parallelizes nothing; the human's send authorizes the cascade). When the work is not decomposable, omit all of this. See `references/runtime-capabilities.md`.
...
- Keep optional operations such as commits, PRs, sidecars, notifications, or external writes out unless they are requested or required for honest Done evidence. Keep reflexive, unbounded fan-out out; but for genuinely decomposable work keep the decomposition structure (discovery rule, per-unit owned surface and evidence, integration check) and, on Codex, the imperative launch directives — read-only subagents for research/review/verification, and write fan-out (partitioned subagents, or `create_thread` per unit when an established project exists) after a phase-0 bootstrap — these are required, not optional (see the next item).
...
- For decomposable work (independent, separately verifiable, share-no-state units — build, review, or research), the Goal carries the decomposition structure (discovery rule, per-unit owned surface and evidence/deliverable, integration check) plus a runtime-sized launch directive. On Claude Code a dynamic-workflow fan-out. On Codex, **flat-imperative** directives naming the tool concretely with **no hedge words** ("when useful"/"only when…" stop them firing), delivered as a `/goal …` line for the user to send (a tool-set goal parallelizes nothing), and following the three rules: subagents (`spawn_agent`) are the default for read-only and partitioned-file write work; `create_thread` only when an established project exists (it needs a resolvable `projectId`); name the tool not its arguments (no `projectId`/`target.type`/schema fields — a non-visible `projectId` causes serial fallback); and on an empty/non-git workspace a phase-0 main-thread bootstrap (git init, build/test scaffold, committed interface contracts) precedes write fan-out. For non-decomposable work all of this is correctly absent
