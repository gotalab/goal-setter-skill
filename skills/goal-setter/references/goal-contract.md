...
Every non-trivial Goal should explicitly allow available governed subagents for separable investigation, source-backed research, validation discovery, test-failure triage, strategy review when progress stalls, and final fresh-context review. Prefer read-only subagents unless an external side effect is required, allowed by policy, and part of end-to-end completion. Keep the authorization short but present. It mainly shapes Claude Code, where the model fans out on its own judgment. On Codex both `spawn` (subagents, read-only) and `create_thread` (own worktree, write work) are gated to the user's own typed request: a goal the skill auto-sets via `create_goal` does not trigger them, but a `/goal …` line the user types does (verified). So on Codex, deliver decomposable goals as a `/goal …` line for the user to send — that one send authorizes the cascade, with the human's send as the trigger.

For parallel fan-out — implementation decomposition, multi-aspect review, or multi-topic research — put the structure in the Goal: a discovery rule for the independent units (state the rule, do not enumerate units unknown at draft time), an owned surface and its own Done evidence or deliverable per unit, item-by-item progress, and a parent integration/synthesis check. The structure works on both runtimes and even when the units are discovered at runtime. Large work often stages as a phased pipeline: (1) parallel read-only research to clarify scope and open questions, (2) parallel write implementation, (3) integrate, (4) parallel adversarial/final verification — use the phases that fit, keeping each phase's units independent with their own evidence. For parallel speed, add a launch directive sized to the runtime. Match the tool to the work type: read-only work (research, multi-aspect review, adversarial/final verification) uses **subagents** (lightweight, no worktree); write work (parallel implementation) uses **`create_thread`** (each unit in its own worktree so concurrent writes do not collide — heavyweight, so reserve it for genuine parallel write units). On Claude Code the run fans out via a dynamic workflow on its own judgment — subagents for read-only stages, worktree isolation for write stages. On Codex, write flat-imperative directives naming the tool concretely: "spawn read-only subagents to research/review/verify <X>"; "use `create_thread` per write unit — own worktree, its own goal scoped to the unit, run in parallel — then integrate in the main thread, autonomously without waiting for the user." Decomposability is *your* draft-time judgment: once you have judged the work decomposable, the Goal **commands** the fan-out unconditionally. Never hedge the directive with "when useful", "if appropriate", "only when file ownership is separable", or similar — those qualifiers make Codex decide it is not useful and run serially (verified, repeatedly). Abstract phrasing ("fresh-context verification", "use subagents") is read as in-context self-work and launches nothing; name the tool. But name the *tool*, not its arguments: do not write `projectId`, `target.type`, or other tool-schema fields into the Goal — a `projectId` the executor cannot see is read as "cannot create a thread" and it falls back to serial (observed); instruct Codex to `create_thread` in the current project/workspace and let the runtime resolve the location. And when the workspace may be empty or not yet git-backed, write the ordering explicitly: the main thread first bootstraps the repo (git init, build/test scaffold, committed cross-module **interface contracts**) — phase 0 — and only then fans out `create_thread` per write unit against that fixed baseline; do not fan out write worktrees into bare ground. The "decompose only when…" conditions are your pre-draft checklist, not words to copy into the Goal. On Codex, deliver this goal as a `/goal …` line for the user to send rather than auto-setting it via `create_goal`: `create_thread`/`spawn` fire only on the user's own typed request, so a tool-set goal parallelizes nothing, while the human sending the line authorizes the whole cascade. When the work is not decomposable, omit all of this. See `references/runtime-capabilities.md`.
...
When the work is decomposable, write the structure into the Goal: a discovery rule for the independent units (state the rule, do not enumerate units you cannot yet name), an owned surface and its own Done evidence or deliverable per unit, item-by-item progress, and a parent integration/synthesis check. This structure is runtime-agnostic and gives the contract its value even with no parallelism. Large work often stages as a phased pipeline — (1) parallel read-only research to clarify scope, (2) parallel write implementation, (3) integrate, (4) parallel adversarial/final verification — so use the phases that fit. For parallel speed, add the launch directive that fits the runtime, matching the tool to the work type: read-only work (research, review, verification) uses subagents (lightweight); write work (implementation) uses `create_thread` (own worktree per unit). On Claude Code the run fans out via a dynamic workflow on its own judgment — subagents for read-only stages, worktree isolation for write stages. On Codex write imperative directives naming the tool concretely: "spawn read-only subagents to research/review/verify <X>"; for each write unit open a separate thread with `create_thread` in its own worktree, set it a goal scoped to its unit, run them in parallel, then integrate in the main thread, autonomously without waiting for the user. Both fire only from the user's own typed `/goal` send, so deliver this goal as a `/goal …` line rather than auto-setting it. When the work is not decomposable, leave all of this out.
...
- Keep optional operations such as commits, PRs, sidecars, notifications, or external writes out unless they are requested or required for honest Done evidence. Keep reflexive, unbounded fan-out out; but for genuinely decomposable work keep the decomposition structure (discovery rule, per-unit owned surface and evidence, integration check) and, on Codex, the imperative launch directives — read-only subagents for research/review/verification, `create_thread` per write unit — these are required, not optional (see the next item).
...
- For decomposable work (independent, separately verifiable, share-no-state units — build, review, or research), the Goal carries the decomposition structure (discovery rule, per-unit owned surface and evidence/deliverable, integration check) plus a runtime-sized launch directive that matches tool to work type: read-only work (research, review, verification) → **subagents**; write work (implementation) → **`create_thread`** (own worktree per unit). On Codex these are **flat-imperative** directives naming the tool concretely, with **no hedge words** — "when useful", "if appropriate", "only when…" would stop them firing — and the goal is delivered as a `/goal …` line for the user to send (a tool-set goal parallelizes nothing). On Claude Code a dynamic-workflow fan-out. For non-decomposable work all of this is correctly absent
- For Codex write fan-out: the directive names the tool but not its arguments (no `projectId`/`target.type`/schema fields — a non-visible `projectId` causes serial fallback), and when the workspace may be empty or not git-backed, a phase-0 main-thread bootstrap (git init, build/test scaffold, committed interface contracts) precedes `create_thread` fan-out
