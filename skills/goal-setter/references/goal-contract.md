...
Every non-trivial Goal should explicitly allow available governed subagents for separable investigation, source-backed research, validation discovery, test-failure triage, strategy review when progress stalls, and final fresh-context review. Prefer read-only subagents unless an external side effect is required, allowed by policy, and part of end-to-end completion. Keep the authorization short but present, and never compress it away: some runtimes (Codex in particular) will not use subagents during a goal run unless the goal text grants it, so implicit agent autonomy or generic "use tools" wording is not enough. Parallel decomposition needs a stronger, separate grant. When the work is decomposable — independent, separately verifiable sub-outcomes that share no state — write an explicit, affirmative spawn instruction into the Goal text, distinct from the read-only investigation preference above: name the independent pieces and tell the runtime to spawn one agent per piece — on Codex each agent gets its own `create_goal` child contract scoped to its files; on Claude Code the run fans out through a dynamic workflow. A single `/goal` does not spawn parallel runs on its own, and Codex will not spawn unless the Goal concretely instructs it, so a vague "may fan out if useful" aside is not enough — it gets dropped in compression or ignored at runtime. When the work is not decomposable, omit this entirely rather than leaving a dormant conditional. See `references/runtime-capabilities.md`.
...
When the work is decomposable, the spawn instruction is not boilerplate to trim — it is the clause that makes parallel execution actually happen, so instantiate it concretely. List the independent pieces, give each an owned surface, and state the per-piece spawn directly: on Codex, "spawn one agent per module, each with its own `create_goal` contract scoped to its files"; on Claude Code, "fan out through a dynamic workflow." Without this the run stays serial, because Codex does not spawn parallel agents unless the Goal says so in concrete terms — the generic read-only "use subagents for investigation" clause does not trigger it. When the work is not decomposable, leave the instruction out.
...
- Keep optional operations such as commits, PRs, sidecars, notifications, or external writes out unless they are requested or required for honest Done evidence. Keep reflexive, unbounded fan-out out — but an explicit parallel-spawn grant for genuinely decomposable work is required, not optional, and must stay (see the next item).
...
- For decomposable work (independent, separately verifiable, share-no-state pieces), an explicit parallel-spawn instruction is present — pieces named, owned surfaces assigned, per-piece spawn stated (Codex: own `create_goal`; Claude Code: dynamic workflow), parallel cap set — not merely the read-only subagent clause; for non-decomposable work it is correctly absent
