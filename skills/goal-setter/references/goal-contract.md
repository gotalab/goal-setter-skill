...
Every non-trivial Goal should explicitly allow available governed subagents for separable investigation, source-backed research, validation discovery, test-failure triage, strategy review when progress stalls, and final fresh-context review. Prefer read-only subagents unless an external side effect is required, allowed by policy, and part of end-to-end completion. Keep the authorization short but present. It mainly shapes Claude Code, where the model fans out on its own judgment. On Codex — verified — subagents and threads launch only on an explicit user request in the prompt, not from Goal text, so the Goal cannot trigger them; the clause still documents intended delegation, but the user must ask.

For parallel decomposition, put only the structure in the Goal — a discovery rule for the independent units, an owned surface and its own Done evidence per unit, item-by-item progress, and a parent integration check over the merged result. The structure works on both runtimes and even when the units are only discovered at runtime, so state the rule rather than enumerating pieces you cannot yet name. Do not write a spawn command into the Goal: it does not trigger Codex (verified), and a `/goal` does not spawn parallel runs on its own. When parallel speed is wanted, Claude Code fans out via a dynamic workflow on its own judgment; Codex needs an explicit user prompt — a short paste-line goal-setter supplies (`spawn` subagents, or `create_thread` for separate threads each with their own goal and worktree). When the work is not decomposable, omit the decomposition structure entirely. See `references/runtime-capabilities.md`.
...
When the work is decomposable, write the decomposition structure into the Goal: a discovery rule for the independent units (state the rule, do not enumerate units you cannot yet name), an owned surface and its own Done evidence per unit, item-by-item progress, and a parent integration check over the merged result. This structure is runtime-agnostic and is what gives the contract its value even with no parallelism. Do not write a spawn command into the Goal — a spawn line in Goal text does not trigger Codex (verified), and a `/goal` does not spawn parallel runs on its own. Parallel speed is a separate, runtime-specific layer: Claude Code fans out via a dynamic workflow on its own judgment; Codex needs an explicit user paste-line (`spawn` subagents, or `create_thread` for separate threads each with their own goal and worktree — see Output Style in `SKILL.md`). When the work is not decomposable, leave the structure out.
...
- Keep optional operations such as commits, PRs, sidecars, notifications, or external writes out unless they are requested or required for honest Done evidence. Keep reflexive, unbounded fan-out and spawn commands out of the Goal — spawn commands do not trigger Codex anyway; for genuinely decomposable work keep the decomposition structure (discovery rule, per-unit owned surface and evidence, integration check), which is required, not optional (see the next item).
...
- For decomposable work (independent, separately verifiable, share-no-state pieces), the Goal carries the decomposition structure — a discovery rule for the units, an owned surface and its own Done evidence per unit, item-by-item progress, and a parent integration check — with no spawn command in the Goal text (which does not trigger Codex); for non-decomposable work the structure is correctly absent. When the user wants Codex parallelism, a short spawn/`create_thread` paste-line is offered separately from the Goal
