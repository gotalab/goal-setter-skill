...
- **States delegation and independent verification.** The goal names what may be delegated and requires a fresh-context check (an independent subagent or equivalent) before Done. Claude Code fans out on its own judgment; on Codex both the `spawn` subagent tool (read-only work) and `create_thread` (write work) fire only from a `/goal` line you send, so the goal is written for you to send rather than auto-set.
- **Structures splittable work for parallelism.** When the outcome breaks into independent, separately verifiable units — a multi-module build, a multi-aspect review, multi-topic research — the goal carries the decomposition structure (a discovery rule, an owned surface and its own checks per unit, an integration check) plus a runtime-sized launch directive, often staged as a phased pipeline: parallel research → parallel implementation → integrate → parallel adversarial/final verification. The tool matches the work type: read-only stages use lightweight `spawn` subagents, write stages use `create_thread` (a separate worktree per unit). Claude Code fans this out via a dynamic workflow on its own judgment; on Codex you send the `/goal …` line and that one action authorizes the whole cascade.
...
When the outcome splits into independent, separately verifiable units — a multi-module build, a multi-aspect review, multi-topic research — the goal carries the decomposition structure: a discovery rule, an owned surface and its own checks per unit, and an integration/synthesis check over the merged result. Large work often stages as a **phased pipeline**: (1) parallel read-only research to clarify scope and open questions, (2) parallel write implementation, (3) integrate, (4) parallel adversarial/final verification. The tool matches the work type — read-only stages use lightweight **subagents** (no worktree); write stages use **`create_thread`** (a separate git worktree per unit, so concurrent writes do not collide). How it runs depends on the runtime:
...
- **Codex** has both tools gated to *your* own typed request, not to a goal the skill sets behind the scenes — so for decomposable work the skill hands you a `/goal …` line carrying imperative directives that name the tool concretely (spawn read-only subagents to research/review/verify; open a `create_thread` per write unit, each in its own worktree with its own goal, run them in parallel, then integrate in the main thread, autonomously). You send that one line; that single action authorizes the whole cascade — subagents, threads, and their per-unit goals.
