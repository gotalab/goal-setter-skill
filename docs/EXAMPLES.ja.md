...
goal-setter が作る Goal 本文の例です。表示を分かりやすくするため `/goal` を付けていますが、発動を依頼された Codex では同じ本文を native Goal 機能で設定し、ユーザーに送り直してもらいません。実際には、先にコードや資料を読み、実在するファイル、確認コマンド、守る条件を入れます。結果に影響しない内容は落とします。
...
画面確認の証拠に効くようにする。独立した Codex タスクと worktree に分けて
並行実行する goal を設定して
```
...
HUD/画面確認の証拠を別々に確認できる作業単位として扱う。ユーザーが別タスク化を
指定しているため、Codex では各単位に
安定した担当範囲、個別の確認方法、理解済みの共有部分、既に使える Git の作業場所が
あるかを先に確認する。どれか欠けるなら直列へ切り替えず、別タスク化できない理由と
必要な判断を報告して止まる。全部そろう場合だけ作業単位ごとに `create_thread` を作る。各スレッドには
