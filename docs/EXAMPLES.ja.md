...
通り、CSV が documented columns と一致し、read-only の独立検証で billing
behavior drift がないと確認できたときだけ。
```
...
build、tests、browser smoke、read-only final verification が揃った時だけ Done。
```
