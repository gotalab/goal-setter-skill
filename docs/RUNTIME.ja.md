...
- item-by-item の進捗
- 親 thread の統合チェック
- Done 前の独立検証。リスクが高い主張に具体的な確認対象がある場合は、崩しにいく検証も含める
...
既存の Codex project で git HEAD が使え、独立して検証できる大きな write unit が複数ある場合、`create_thread` worktree fan-out を必須にします。
...
`spawn_agent` は読み取り系に使います。調査、複数観点レビュー、崩しにいくレビュー、最終検証、ログ解析、既存挙動の把握など、main thread の context を汚しやすい作業に向いています。subagent は証拠、反証、不確実性、抜け、読み取り結果を返し、親 agent が統合、書き込み判断、最終判断を持ちます。
...
- 長い run の進捗ルール
- stop conditions
- independent verification。高リスクな主張に具体的な対象がある場合は、崩しにいく検証
- final report
