...
description: Compact version of goal-setter — one SKILL.md plus a length-check helper. Turns a rough request into a short, evidence-backed /goal completion contract with the same core gates (intended-outcome image, clarification, binary evidence-bounded Done, fresh-context verification), the verified Codex/Claude Code parallel mechanics, and lightweight execution-notes for long runs, without the full reference set. Use for long-running implementation, migration, refactor, debugging, research, or review work when you want the essentials. For the full readiness rubric and GOAL.md run scaffolding, use the goal-setter skill instead. Not for one-turn edits, ordinary Q&A, or subjective work with no rubric.
---

# Goal Setter (Lean)

Turn a rough request into a compact `/goal` completion contract — the shortest condition that still pins the requested outcome. This is the compact version of `goal-setter` (one SKILL.md plus a length-check helper); for the full reference set, the readiness rubric, and `GOAL.md` run scaffolding, use that skill.
