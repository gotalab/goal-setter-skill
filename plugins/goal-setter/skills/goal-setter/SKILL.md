...
- State intended delegation in the Goal, sized to the runtime. Claude Code fans out subagents/workflows on its own judgment. On Codex, `create_thread` fires from an imperative Goal instruction (verified) — so for decomposable work (build, review, or research) write a `create_thread` directive into the Goal itself: per-unit thread, own worktree and goal, run in parallel, integrate in the main thread, autonomously. The `spawn` subagent tool is gated to explicit user requests, so it stays a user-prompt-only fallback (see Output Style).
