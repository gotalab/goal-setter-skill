...
For each piece, name the owned surface, cap the parallel count and retries, and make the parent Done depend on every piece's evidence plus the integration check. When the work is decomposable, write this as an explicit, affirmative spawn instruction in the Goal text — name the pieces and state the per-piece spawn ("spawn one agent per module, each with its own `create_goal` contract" on Codex; "fan out through a dynamic workflow" on Claude Code). Do not water it down to a conditional "may fan out if useful" aside: that watered-down form is exactly what fails to fire, because Codex spawns parallel agents only when the Goal concretely instructs it, and the bloat pass tends to drop a vague optional clause. When the work is not decomposable, omit the grant entirely rather than leaving a dormant conditional.
