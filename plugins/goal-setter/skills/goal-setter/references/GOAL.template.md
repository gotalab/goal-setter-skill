...
This work splits into independent, separately verifiable sub-outcomes that share no state. Treat each discovered unit as a separately verifiable piece that owns only its files, work unit by unit, and gate Done on every unit's evidence plus an integration check that confirms the merged result. Parallel speed is a runtime matter, not a Goal command: on Claude Code the run may fan out through a dynamic workflow on its own judgment; on Codex, parallelism is launched only by an explicit user prompt (`spawn` subagents, or `create_thread` for separate threads each with their own goal and worktree), not from this text. (Drop this whole paragraph when the work is not decomposable — interlocking files, a single-cause investigation, or a serially tuned metric — and keep one write contract, using read-only subagents only for separable investigation.)
