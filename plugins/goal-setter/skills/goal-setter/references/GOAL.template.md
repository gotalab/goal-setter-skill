...
This work splits into independent, separately verifiable sub-outcomes that share no state. Treat each discovered unit as a separately verifiable piece that owns only its files, work unit by unit, and gate Done on every unit's evidence plus an integration check that confirms the merged result. Large work often stages as a phased pipeline: (1) parallel read-only research to clarify scope and open questions, (2) parallel write implementation, (3) integrate, (4) parallel adversarial/final verification — use the phases that fit. Match the tool to the work type: read-only work (research, multi-aspect review, adversarial/final verification) uses **subagents** (lightweight, no worktree); write work (parallel implementation) uses **`create_thread`** (each unit in its own worktree). For parallel speed: on Claude Code the run may fan out through a dynamic workflow on its own judgment — subagents for read-only stages, worktree isolation for write stages. On Codex, orchestrate from this main thread — spawn read-only subagents to research/review/verify, and for each write unit use the `create_thread` tool to open a separate thread in its own worktree with a goal scoped to only that unit, run them in parallel, then integrate and validate in this main thread, autonomously without waiting for further user instruction. If this workspace is empty or not yet git-backed, first bootstrap it from this main thread — git init, build/test scaffold, and committed cross-module interface contracts — before fanning out `create_thread` worktrees against that baseline. Name the tool, not its arguments: do not specify `projectId` or other tool-schema fields; create threads in the current project/workspace and let the runtime resolve the location — a project id you cannot see is not a reason to fall back to serial work. (Drop this whole paragraph when the work is not decomposable — interlocking files, a single-cause investigation, or a serially tuned metric — and keep one write contract, using read-only subagents only for separable investigation.)
