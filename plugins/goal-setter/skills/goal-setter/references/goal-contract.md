...
Compress the image into a single context line that opens the Goal text: what the outcome serves and for whom. Knowing the reason behind the request, not only the request, lets the executor connect forks and tradeoffs to the real intent instead of guessing it. Keep the rest of the image out of the Goal; it shapes the objective, evidence, validation, constraints, and Done.
